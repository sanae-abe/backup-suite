...
fn test_backup_and_restore_with_integrity_verification() {
    ...
    // 復元されたファイルの内容を確認
    // 注: ディレクトリバックアップではディレクトリ名も保持されるため、test-category/source/ 配下に復元される
    let restored1 = fs::read_to_string(restore_dir.join("test-category/source/test1.txt")).unwrap();
    assert_eq!(restored1, "test content 1");

    let restored2 =
        fs::read_to_string(restore_dir.join("test-category/source/subdir/test2.txt")).unwrap();
