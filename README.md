...
#### 2. バックアップ保存先の設定
**重要**: 最初にバックアップの保存先を設定しましょう

```bash
# Google Driveに保存する場合（推奨）
...
# 外付けハードディスクに保存する場合
backup-suite config set storage.path "/Volumes/外付けHDD名/backup-storage"

# ローカルディスクに保存する場合
backup-suite config set storage.path "/Users/あなたのユーザー名/backup-storage"
```

#### 3. 保存先の選び方
| 保存先 | メリット | デメリット | おすすめ度 |
|--------|----------|------------|------------|
| **Google Drive** | 自動クラウド同期、容量大 | 同期に時間がかかる場合 | ⭐⭐⭐⭐⭐ |
| **外付けHDD** | 高速、大容量、安い | 持ち運び必要、故障リスク | ⭐⭐⭐⭐ |
| **ローカルディスク** | 最高速、設定簡単 | 本体故障時にバックアップも失う | ⭐⭐ |
