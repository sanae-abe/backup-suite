...
### 🔐 **軍事レベルの暗号化保護**
**何ができる？**: 銀行や政府機関と同じレベルの暗号化で、大切なファイルを完全に保護できます

#### 🛡️ 暗号化の仕組み
- **AES-256-GCM暗号化**: 現在最も強力な暗号化方式を採用
- **解読は事実上不可能**: スーパーコンピューターでも数千年かかる強度
- **パスワードベース暗号化**: あなたのパスワードがないと誰も開けません

#### 🔒 こんな時に重要
- **個人情報や機密書類**のバックアップ
- **パソコンが盗まれた時**でもデータは安全
- **クラウドに保存**しても第三者は絶対に見れない
- **USBやハードディスク紛失**でも情報漏洩なし

#### 💡 使い方
```bash
# パスワード付きで完全暗号化バックアップ
backup-suite run --encrypt --password "超安全なパスワード123!"

# 特定の重要ファイルのみ暗号化
backup-suite add ~/機密書類 --priority high --encrypt
```
